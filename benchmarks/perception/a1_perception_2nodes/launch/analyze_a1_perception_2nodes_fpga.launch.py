...
def msgsets_from_ctf_vtf_traces(ctf_trace, vtf_trace):
    ...
    for index in range(len(all_msgs_sorted)):
        if all_msgs_sorted[index].event.name in target_chain:  # optimization
            ...
            if (
                chain_index == 0
                and all_msgs_sorted[index].event.name == target_chain[chain_index]
            ):
            ...
            # last one
                ...
                all_msgs_sorted[index].event.name == target_chain[chain_index]
            ...
            # altered order
                ...
                all_msgs_sorted[index].event.name in target_chain
                # and all_msgs_sorted[index].event.common_context_field.get("vpid")
                # == vpid_chain
            ):
...
def traces(msg_set):
    ...
    add_durations_to_figure(
        fig,
        target_chain_layer[4],  # index used in here
                                # should match with the
                                # one from the callback_start
        [(callback_start, callback_start + duration, duration)],
        "lightgray",
    ...
    add_durations_to_figure(
        fig,
        target_chain_layer[20], # index used in here
                                # should match with the
                                # one from the callback_start
        [(callback_start, callback_start + duration, duration)],
        "lightgray",
    ...
    add_durations_to_figure(
        fig,
        target_chain_layer[13], # index used in here
                                # should match with the
                                # one from the callback_start
        [(callback_start, callback_start + duration, duration)],
        "whitesmoke",
    ...
    add_durations_to_figure(
        fig,
        target_chain_layer[14], # index used in here
                                # should match with the
                                # one from the callback_start
        [(callback_start, callback_start + duration, duration)],
        "seashell",
    ...
    add_durations_to_figure(
        fig,
        target_chain_layer[21], # index used in here
                                # should match with the
                                # one from the callback_start
        [(callback_start, callback_start + duration, duration)],
        "whitesmoke",
