...
def analyze_planning(argv):
    ...
    trace_path = os.path.join(args.trace_path, "d7_dual_arm_static_avoidance_distributed")
    ...
    ba = BenchmarkAnalyzer('d7_dual_arm_static_avoidance_distributed', hardware_device_type)
...
def analyze_tf2_operations_for_workstation(argv):
    ...
    # Get the values of the arguments
    hardware_device_type = args.hardware_device_type
    trace_path = os.path.join(args.trace_path, "d7_dual_arm_static_avoidance_workstation")
    metrics_string = args.metrics
    metrics_elements = [element.strip() for element in metrics_string.strip("[]").split(",")]
    metrics = json.loads(json.dumps(metrics_elements))
    integrated = args.integrated
    ...
    # Instantiate the class
    ba = BenchmarkAnalyzer('d7_dual_arm_static_avoidance_workstation', hardware_device_type, tf_tree)
...
def analyze_tf2_operations_for_embedded(argv):
    tf_tree = FrameHierarchy()
    tf_tree.add_frame("world", "link_base")
    ...
    # Get the values of the arguments
    hardware_device_type = args.hardware_device_type
    trace_path = os.path.join(args.trace_path, "d7_dual_arm_static_avoidance_embedded")
    metrics_string = args.metrics
    metrics_elements = [element.strip() for element in metrics_string.strip("[]").split(",")]
    metrics = json.loads(json.dumps(metrics_elements))
    integrated = args.integrated
    ...
    # Instantiate the class
    ba = BenchmarkAnalyzer('d7_dual_arm_static_avoidance_embedded', hardware_device_type, tf_tree)
    ba.set_trace_sets_filter_type(filter_type="UID")
...
def generate_launch_description():
    ...
    analyzer = ExecuteProcess(
        cmd=[
            'python3', "src/benchmarks/benchmarks/manipulation/d7_dual_arm_static_avoidance/launch/analyze_d7_dual_arm_static_avoidance_distributed.launch.py",
...
def merge_workstation_and_embedded_ctf_directories(argv):
    ...
    # Check that both workstation and embedded trace files exist
    base_dir_name = "d7_dual_arm_static_avoidance"
    workstation_trace_path = os.path.join(trace_path, base_dir_name + "_workstation")
    embedded_trace_path = os.path.join(trace_path, base_dir_name + "_embedded")
    output_trace_path = os.path.join(trace_path, base_dir_name + "_distributed")

    if not os.path.isdir(workstation_trace_path):
        raise Exception(workstation_trace_path + " directory was not found")
    elif not os.path.isdir(embedded_trace_path):
        raise Exception(embedded_trace_path + " directory was not found")
    elif os.path.isdir(output_trace_path):
        raise Exception(output_trace_path + " already exists and results would be appended to it. Delete the directory and try again.")

    print("Merging workstation and embedded trace files using babeltrace2...")

    merge_ctf_directories_command = f"babeltrace2 {workstation_trace_path} {embedded_trace_path} --output={output_trace_path} --output-format=ctf"
    if (os.system(merge_ctf_directories_command)):
        raise Exception(f"There was an error while merging {workstation_trace_path} and {embedded_trace_path}")

    print(f"Trace files have been merged in {output_trace_path}")

if __name__ == '__main__':

    merge_workstation_and_embedded_ctf_directories(sys.argv[1:])
    ...
    # TF2 runs on both devices at the same time
    # It doesn't make sense to merge this data and analyze it
    analyze_tf2_operations_for_workstation(sys.argv[1:])
